...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        # self.attention_weights = torch.ones(batch_size, seq_len, self.num_heads, self.one_sided_attn_window_size * 2 + 1)
    ...
    def forward(
        ...
        if attention_weights is not None:
            attn_scores += attention_weights[self.layer_id] #({batch_size}, {seq_len}, {self.num_heads}, {self.one_sided_attn_window_size * 2 + 1})
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, is_global_attn, output_attentions, attention_weights)
